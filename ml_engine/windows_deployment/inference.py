# ---------------------------------------------------------------------------
# DEPLOY (Windows / manual copy): place this file in the ml_engine ROOT as
#   inference.py   (overwrite the existing one next to models\, requirements.txt)
# Example:  copy ...\windows_deployment\inference.py  C:\ml_engine\inference.py
# Then set ML_WAV2VEC_LOCAL to your local Wav2Vec folder (run_ml_server.ps1 does
# this automatically if models\wav2vec2-large-960h exists).
# ---------------------------------------------------------------------------
...
class DLInferenceLarge:
    ...
    def _load_model(self, model_path):
        ...
        checkpoint = torch.load(model_path, map_location=self.device, weights_only=False)
        state = _extract_linear_head_state_dict(checkpoint)
